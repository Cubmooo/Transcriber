...
void NoteWidget::paintEvent(QPaintEvent *)
{
    ...
    for (size_t i = 0; i < notes.size(); ++i)
    {
        ...
        while (endIndex < notes.size() &&
            notes[endIndex].first == notePosition){
            endIndex++;
        }

        if (endIndex > i + 1){
            noteLength = notes[endIndex - 1].second - beat;
        }

        if (i > 0 && notes[i - 1].first == notePosition){
            continue;
        }
        ...
        {
            if (noteLength >= 4.0)
                crochet = QString(SMuFL::semibreve);
            else if (noteLength >= 2.0)
                crochet = QString(SMuFL::upStemMinim);
            else if (noteLength >= 1.0)
                crochet = QString(SMuFL::upStemCrochet);
            else if (noteLength >= 0.5)
                crochet = QString(SMuFL::upStemQuaver);
            else
                crochet = QString(SMuFL::upStemSemiquaver);
        }
        else
        {
            if (noteLength >= 4.0)
                ...
                crochet = QString(SMuFL::downStemMinim);
            else if (noteLength >= 1.0)
                crochet = QString(SMuFL::downStemCrochet);
            else if (noteLength >= 0.5)
                crochet = QString(SMuFL::downStemQuaver);
            else
                crochet = QString(SMuFL::downStemSemiquaver);
        }
